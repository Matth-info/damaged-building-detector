...
from training.functional_auto_encoder import train, find_threshold
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train an Auto-Encoder model for Cloud Coverage Detection")
    ...
    parser.add_argument("--origin_dir", type=str, default="./data/data_samples/Cloud_DrivenData/final/public", help="Local path to Cloud Dataset.")
...
def main():
    ...
    train_x, train_y , val_x, val_y = prepare_cloud_segmentation_data(folder_path = origin_dir, train_share = 0.8, seed=42)
    ...
    # Find a Threshold 
    print("Find a Reconstruction loss threashold on Test Set")
    threshold, _, _ = find_threshold(model, 
                                    data=data_train,
                                    loss_fn=criterion,
                                    device=device, 
