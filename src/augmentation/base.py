...
class Augmentation_pipeline :
    def __init__(self, 
        ...
        """
        assert package in ["albumentations", "torchvision"], "Package must be 'albumentations' or 'torchvision'"
        assert mode in ["train", "val", "test"], "Dataset must be 'train','val' or 'test'"
        self.mode = mode
    ...
    def save_pipeline(self, data_format: str = "json", folder_path: str = None) -> None:
        ...
        if self.package == "albumentations":
            ...
            A.save(self.pipeline, filepath_or_buffer=f"{filepath}.{data_format}", data_format=data_format)
    ...
    def load_pipeline(cls, filepath: str = None):
        ...
        assert data_format in ["yaml", "json"], "Only 'json' and 'yaml' formats are supported."
    ...
    def debug(self, image_1: np.ndarray = None, image_2: np.ndarray = None, mask: np.ndarray = None, n_samples: int = 10):
        ...
        for i in range(n_samples):
            ...
            image = np.transpose(renormalize_image(
                results["image"], mean=self.mean, std=self.std, device="cpu").numpy(), (1, 2, 0))

            vis_image_2 = None
            if image_2 is not None:
                vis_image_2 = np.transpose(renormalize_image(
                    results["post_image"], mean=self.mean, std=self.std, device="cpu").numpy(), (1, 2, 0))
