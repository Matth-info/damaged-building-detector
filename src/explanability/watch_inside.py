...
def visualize_feature_maps(x1, x2, preds, ground_truth, feature_maps_pre, feature_maps_post, feature_maps_decoder, k=10, num_cols=6, mean=None, std=None, output_path=None):
    """
    ...
    - output_path (str)
    """
    assert set(feature_maps_pre.keys()) == set(feature_maps_post.keys()), "Feature maps mismatch between pre and post encoders"
    
    # Select k random layers from pre and corresponding post
    feature_maps_pre_list = list(feature_maps_pre.items())
    selected_pre_layers_ids = sorted(random.sample(list(range(len(feature_maps_pre_list))), min(k, len(feature_maps_pre_list))))
...
def main():
    ...
    for batch in dataloader:
        pre_image, post_image, mask = batch["pre_image"].to(device, dtype=torch.float32) , batch["post_image"].to(device, dtype=torch.float32), batch["mask"]
