...
def custom_infer_collate_is_siamese(batch: list[dict]) -> dict:
    """Define a Custom Pytorch Collate function handling the aggregation of filenames (for naming prediction) and share the GeoTiff profile to the predictions files.
...
def custom_infer_collate(batch: list[dict]) -> dict:
    """Define a Custom Pytorch Collate function handling the aggregation of filenames (for naming prediction) and share the GeoTiff profile to the predictions files.
...
def save_batch_prediction(
    ...
    for i, (pred, basename) in enumerate(zip(mask_predictions, filenames)):
        file_path = Path(pred_folder_path) / f"{basename}.{mode.lower()}"
...
def batch_inference(
    ...
    *,
    ...
    ----
        ...
        is_siamese (bool): If model takes both pre/post images.
    ...
    """
    if save:
        Path.mkdir(pred_folder_path, exist_ok=True)
    # Torch Compile for speedup computation
    model.to(device)
    model.eval()
    with torch.no_grad(), tqdm(dataloader, desc="Inference Running", unit="batch") as t:
        for batch in t:
