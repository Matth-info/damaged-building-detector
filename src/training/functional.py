...
from .utils import log_metrics, log_images_to_tensorboard, save_model, load_model, display_metrics
import time
import numpy as np
...
def training_step(model, batch, loss_fn: _Loss, optimizer : optim, metrics: List[Callable], step_number: int):
    ...
    if step_number % training_log_interval == 0:
        ...
        tp, fp, fn, tn = get_stats(output=out, target=y.long(), mode=mode, num_classes=num_classes)
        for metric in metrics:
            metric_name = metric.__name__
            metric_value = metric(tp, fp, fn, tn, class_weights=None, reduction=reduction) # (N,C)
...
def training_epoch(model: nn.Module, train_dl: DataLoader, loss_fn: _Loss, metrics: List[Callable],  optimizer : optim , scheduler : optim.lr_scheduler,epoch_number:int, writer:SummaryWriter):
    ...
    for step, batch in enumerate(train_dl): 
        loss_t, metrics_step = training_step(model=model, batch=batch, loss_fn=loss_fn, metrics=metrics, optimizer=optimizer, step_number=step)
        # Accumulate loss
...
def validation_epoch(
    ...
    metric_totals = {metric.__name__: 0 for metric in metrics}  # Initialize totals for each metric

    model.eval()  # Set model to evaluation mode

    for step, batch in enumerate(valid_dl):
        batch_size = batch["image"].size(0)
        vloss , metrics_step = validation_step(model, batch, loss_fn=loss_fn, metrics=metrics)
...
def train(
    ...
    with SummaryWriter(log_dir) as writer:
        for epoch in range(nb_epochs):
            ...
            epoch_loss, epoch_metrics = training_epoch(model, train_dl, loss_fn=loss_fn, metrics=metrics, optimizer=optimizer_ft, scheduler=lr_scheduler, epoch_number=epoch, writer=writer)

            epoch_vloss, epoch_vmetrics  = validation_epoch(model=model,
                                                         valid_dl=valid_dl,
                                                         loss_fn=loss_fn,
                                                         metrics=metrics, 
                                                         epoch_number=epoch)
            ...
            log_metrics(writer, metrics=epoch_metrics, epoch_number=epoch, phase="training")
            log_metrics(writer, metrics=epoch_vmetrics, epoch_number=epoch, phase="validation")
