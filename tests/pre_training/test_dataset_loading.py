...
def test_load_siamese_datasets(dataset_cls, kwargs, expected_shapes):
    ...
    assert all(key in sample for key in ["pre_image", "post_image"]), f"{dataset_cls.__name__} sample missing required keys!"
    
    assert sample["pre_image"].shape == image_shape, f"Expected shape {image_shape}, but got {sample['pre_image'].shape}"
    assert sample["post_image"].shape == image_shape, f"Expected shape {image_shape}, but got {sample['post_image'].shape}"

    if "pre_mask" in sample.keys():
        assert sample["pre_mask"].shape == mask_shape, f"Expected shape {mask_shape}, but got {sample['pre_mask'].shape}"
    if "post_mask" in sample.keys():
         assert sample["post_mask"].shape == mask_shape, f"Expected shape {mask_shape}, but got {sample['post_mask'].shape}"
